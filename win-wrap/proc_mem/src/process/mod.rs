...
impl Process {
    ...
    pub fn with_pid(pid: u32) -> Result<Self, ProcMemError> {
        ...
        if !process32first(&h_snap, &mut pe32) {
            return Err(ProcMemError::IterateSnapshotFailure);
        }
    ...
    pub fn module(&self, name: &str) -> Result<Module, ProcMemError> {
        ...
        loop {
            let module_name = String::from_utf16_lossy(&me32.szModule)
                .trim_end_matches('\u{0}')
                .to_string();

            if module_name.eq(name) {
                let module_path = String::from_utf16_lossy(&me32.szExePath)
                    .trim_end_matches('\u{0}')
                    .to_string();
